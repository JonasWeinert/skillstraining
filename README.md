...
## Materialize Demo Outputs Without Stata

Facilitators can regenerate the synthetic CSVs with Python:

```bash
/Users/jonas/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 demoporject_manage/scripts/00_materialize_demo_outputs.py
```

Participants do not need Python if `data/raw/*.csv` is already included. Figures
used by the PowerPoint exercise are generated by Stata via
